  ...
  return (
    <main className='bg-background text-foreground'>
      ...
      <div className='mt-6 w-full'>
        <PropertySlideshow images={property.images} propertyTitle={property.title} />
      </div>
            ...
            <div className='space-y-6 p-6 md:p-8'>
              <div className='space-y-4'>
                ...
                <div className='grid gap-4 border-y border-black/10 py-5 sm:grid-cols-3'>
                  ...
                  <div>
                    <p className='text-xs uppercase tracking-wide text-zinc-500'>Zimmer</p>
                    <p className='text-2xl font-bold text-[#18181b]'>{property.rooms}</p>
                  </div>
                  <div>
                    <p className='text-xs uppercase tracking-wide text-zinc-500'>
                      Wohnfläche
                  ...
                  <dl className='grid gap-3 sm:grid-cols-2'>
                    {amenities.map((amenity) => {
                      return (
                        ...
                        >
                          <dt className='font-medium text-zinc-700'>{amenity.label}</dt>
                          <dd className='font-semibold text-[#18181b]'>{amenity.value}</dd>
                        </div>
                      )
                    })}
                  </dl>
                ...
                <dl className='divide-y divide-black/10 rounded-xl border border-black/10'>
                  {detailFields.map((field) => {
                    return (
                      ...
                      >
                        <dt className='font-medium text-zinc-600'>{field.label}</dt>
                        <dd className='font-medium text-[#18181b]'>{field.value}</dd>
                      </div>
                    )
                  })}
                </dl>
                  ...
                  <dl className='divide-y divide-black/10 rounded-xl border border-black/10'>
                    {property.yearBuilt ? (
                      <div className='grid gap-2 px-4 py-3 text-sm sm:grid-cols-[minmax(0,220px)_1fr]'>
                        ...
                        <dd className='font-medium text-[#18181b]'>{property.yearBuilt}</dd>
                      </div>
                    ) : null}
                    {property.energyCertificateType ? (
                      <div className='grid gap-2 px-4 py-3 text-sm sm:grid-cols-[minmax(0,220px)_1fr] odd:bg-[#f4f4f5]'>
                        <dt className='font-medium text-zinc-600'>Energieausweistyp</dt>
                    ...
                    {property.energyCertificateValidUntil ? (
                      <div className='grid gap-2 px-4 py-3 text-sm sm:grid-cols-[minmax(0,220px)_1fr] odd:bg-[#f4f4f5]'>
                        <dt className='font-medium text-zinc-600'>Gültig bis</dt>
                        <dd className='font-medium text-[#18181b]'>
                          {property.energyCertificateValidUntil}
                        </dd>
                      </div>
